...
wp_enqueue_script( 'ihc-user-membership-management', IHC_URL . 'assets/js/user-membership-management.js', ['jquery'], '13.5', false );
...
$payments_available = ihc_get_active_payments_services();
    if ( $data['subscriptions'] !='' && $data['subscriptions'] ){
      ...
      foreach ( $data['subscriptions'] as $subscriptionData ){
        ...
        $levelData = \Indeed\Ihc\Db\Memberships::getOne( $subscriptionData['level_id'] );
        if ( empty( $levelData ) ){
          continue;
        }
...
wp_register_script( 'ihc-stripe-connect-change-card', IHC_URL . 'assets/js/stripe-connect-change-card.js', [ 'jquery' ], '13.5' );
