	...
	<?php
		...
		switch ($subtab):
			...
			<?php
				...
				if ($data && count($data)){
											...
											// js
											wp_enqueue_script( 'ihcdatabse', IHC_URL . 'admin/assets/js/datatables/datatables.min.js', ['jquery'], '13.5' );
											wp_enqueue_script( 'ihcdatabse-buttons', IHC_URL . 'admin/assets/js/datatables/dataTables.buttons.min.js', ['jquery'], '13.5' );
											wp_enqueue_script( 'ihcdatabse-colvis', IHC_URL . 'admin/assets/js/datatables/buttons.colVis.min.js', ['jquery'], '13.5' );
											// iump datatable functions
											wp_register_script( 'ihc-table', IHC_URL . 'admin/assets/js/table.js', ['jquery'], '13.5' );
											// setting up the variables
											global $wp_version;
											$pageState = get_option( 'ihc_datatable_state_for-car_entire_url', false );
											if ( $pageState ){
											    $pageState = stripslashes($pageState);
											} else {
											    $pageState = '';
											}
											if ( version_compare ( $wp_version , '5.7', '>=' ) ){
													...
													wp_add_inline_script( 'ihc-table', "var iump_datatable_state='$pageState';" );
											} else {
													...
													wp_localize_script( 'ihc-table', "iump_datatable_type", $tableDataType );
													wp_localize_script( 'ihc-table', "iump_datatable_state", $pageState );
											}
		...
		<?php
				...
				if ($data && count($data)){
											...
											// setting up the variables
											global $wp_version;
											$pageState = get_option( 'ihc_datatable_state_for-car_url_word', false );
											if ( $pageState ){
											    $pageState = stripslashes($pageState);
											} else {
											    $pageState = '';
		...
		case 'post_types':
			...
			<?php
				...
				if ($data && count($data)){
								...
								// setting up the variables
								global $wp_version;
								$pageState = get_option( 'ihc_datatable_state_for-posts', false );
								if ( $pageState ){
								    $pageState = stripslashes($pageState);
								} else {
								    $pageState = '';
...
', 'ihc');?></p>
			...
			// setting up the variables
			global $wp_version;
			$pageState = get_option( 'ihc_datatable_state_for-car_cats', false );
			if ( $pageState ){
			    $pageState = stripslashes($pageState);
			} else {
			    $pageState = '';
	...
	case 'files':
			...
			<?php
				...
				if ($data && count($data)){
						...
						// setting up the variables
						global $wp_version;
						$pageState = get_option( 'ihc_datatable_state_for-car_files', false );
						if ( $pageState ){
						    $pageState = stripslashes($pageState);
						} else {
						    $pageState = '';
