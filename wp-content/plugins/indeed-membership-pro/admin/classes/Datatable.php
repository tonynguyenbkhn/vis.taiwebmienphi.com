...
class Datatable
{
          ...
          foreach ( $orders as $order ){
              ...
              if ( $taxesOn ){
                  ...
                  if (isset($firstChage) && $firstChage == true && isset($firstAmount)){
                      ...
                      $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($netAmount) );
                  } else {
                      $base_price = $orderMeta->get( $order['id'], 'base_price' );
                      if ( $base_price !== null ){
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($base_price) );
                      } else if ( $taxes != false ){
                        $netAmount = $order['amount_value'] - $taxes;
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($netAmount) );
                      } else {
                        $array['net_amount'] = ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($order['amount_value']) );
                      }
                  }
                  ...
                  if ( $taxes === false || $taxes === '' ){
                      ...
                      $array['taxes'] = ihc_format_price_and_currency( esc_html($order['amount_type']), $taxes );
                  }

              }

              // total amount
              $array['amount_value'] = '<span class="order-total-amount">' . ihc_format_price_and_currency( esc_html($order['amount_type']), esc_html($order['amount_value']) ) . '</span>';
    ...
    <span class ="iump-btns ihc-delete-link ihc-js-admin-coupons-delete-coupon" data-delete_message="' . esc_html__( 'Are You sure You wish to delete this coupon?', 'ihc' ) . '" data-id="' . esc_attr($coupon['id']) . '" >' . esc_html__('Delete', 'ihc') . '</span>
              ...
              if ($coupon['settings']['discount_type']=='percentage'){
                ...
                $array['discount'] = ihc_format_price_and_currency( esc_html( get_option( 'ihc_currency' ) ), $array['discount'] );
              }
              $array['id'] = $coupon['id'];
